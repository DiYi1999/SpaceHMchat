...
save_path = 'E:\OneDrive\DATA\Our_Exp_Data\XJTU-SPS Dataset\QandA_Dataset_4LLM\ChatTime_Format'
...
def feature_exact_and_str(sample, sensor_dict, sensor_list):
    """
    ...
    # 为了让大模型理解数字，字典里面那些numpy全部转化为str，每个数字之间用### ###连接，也就是每个数字用###包围后用空格连接
    # 同时，为了让大模型识别###number###格式，它的分词库里只有一堆保留四位小数的词，所以这里所有数据也都要保留四位小数，多的裁，少的补0
    format_to_4_decimal = np.vectorize(lambda x: f"{x:.4f}") # 批量将数值转换为保留四位小数的字符串
    for key in observed_feature_dict.keys():
        observed_feature_dict[key] = format_to_4_decimal(observed_feature_dict[key])
        # observed_feature_dict[key] = observed_feature_dict[key].tolist()
    for key in temporal_feature_dict.keys():
        temporal_feature_dict[key] = format_to_4_decimal(temporal_feature_dict[key])
    for key in frequency_feature_dict.keys():
        frequency_feature_dict[key] = format_to_4_decimal(frequency_feature_dict[key])

    for key in temporal_feature_dict.keys():
        temporal_feature_dict[key] = '###' \
                                     + '### ###'.join(np.char.mod('%s', temporal_feature_dict[key])) \
                                     + '###'
    for key in frequency_feature_dict.keys():
        frequency_feature_dict[key] = '###' \
                                      + '### ###'.join(np.char.mod('%s', frequency_feature_dict[key])) \
                                      + '###'
    for key in observed_feature_dict.keys():
        observed_feature_dict[key] = '###' \
                                     + '### ###'.join(np.char.mod('%s', observed_feature_dict[key])) \
                                     + '###'
...
def from_data_get_json(data, i, JSON_output, lag, fault_dict, sensor_dict, sensor_list):
    """
    ...
    for j in range(len(data) - lag):
        ...
        temporal_feature_dict, frequency_feature_dict, observed_feature_dict = feature_exact_and_str(sample, sensor_dict, sensor_list)
...
def from_csv_get_json(json_list, read_path, if_train, if_normalized):
    ...
    for i in range(1, len(fault_dict)):
        ...
        # 变成json，加入json文件
        json_list = from_data_get_json(output_data, i, json_list, lag, fault_dict_chinese, sensor_dict, sensor_list)
