...
def temporal_feature_extract(X):
    ...
    # 创建成字典
    feature_dict = {'时域平均值': X_mean.numpy(), '时域绝对平均值': X_abs_mean.numpy(), '时域方差': X_var.numpy(),
                    '时域最大值': X_max.numpy(), '时域最小值': X_min.numpy(), '时域标准差': X_std.numpy(),
                    '时域方根幅值': X_smra.numpy(), '时域均方根值': X_rms.numpy(), '时域峰值': X_p.numpy(),
                    '时域峰-峰值': X_ptp.numpy(), '时域峰值因子': X_CrestFactor.numpy(),
                    '时域波形因子': X_WaveFactor.numpy(), '时域脉冲因子': X_ImpulseFactor.numpy(),
                    '时域裕度因子': X_ClearanceFactor.numpy(), '时域偏度': X_Skewness.numpy(),
...
def frequency_feature_extract(X, exp_frequency=1):
    ...
    # 创建成字典
    feature_dict = {'频域均值': F_12.detach().numpy(), '频域均方根值': F_13.detach().numpy(), '频域偏度': F_14.detach().numpy(),
                    '频域峰度': F_15.detach().numpy(), '频率重心': F_16.detach().numpy(), '频率根方差': F_17.detach().numpy(),
                    '频率均方根': F_18.detach().numpy(), '平均频率': F_19.detach().numpy(), '频率稳定系数': F_20.detach().numpy(),
                    '变异系数': F_21.detach().numpy(), '频率偏度': F_22.detach().numpy(), '频率峰度': F_23.detach().numpy(),
