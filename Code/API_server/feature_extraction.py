...
def temporal_feature_extract(X):
    ...
    '绝对平均值 Absolute mean value：(batch_size, node_num)'
    X_var = torch.var(X, dim=2)
    '方差 Variance：(batch_size, node_num)'
    X_max = torch.max(X, dim=2)[0]
    '最大值：(batch_size, node_num)'
    X_min = torch.min(X, dim=2)[0]
    '最小值：(batch_size, node_num)'
    ...
    # return X_mean, X_abs_mean, X_var, X_max, X_min, X_std, X_smra, X_rms, X_p, X_ptp, \
    #     X_CrestFactor, X_WaveFactor, X_ImpulseFactor, X_ClearanceFactor, X_Skewness, X_Kurtosis
...
def frequency_feature_extract(X, exp_frequency=1):
    ...
    """
    # 如果不是Tensor而是numpy或者dataframe，先转成Tensor
    data_type = type(X)
    if not isinstance(X, torch.Tensor):
        if data_type == pd.DataFrame:
            X = torch.tensor(X.values, dtype=torch.float32)
    ...
    # X_fft = torch.fft.fft(X, n=n, dim=2)
    # 'FFT：(batch_size, node_num, lag)'
    ...
    'F_15：功率谱峭度(batch_size, node_num)'
